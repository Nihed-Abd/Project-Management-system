  ...
  return (
    <div className={`p-6 ${isDark ? 'text-white' : 'text-gray-800'}`}>
      <div className="flex justify-between items-center mb-6">
        <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>User Management</h1>
        
        {/* Add New Client Button */}
        <button
          onClick={addNewClient}
          className="flex items-center bg-coquelicot hover:bg-coquelicot-600 text-white py-2 px-4 rounded-md space-x-2"
          disabled={addingUser}
        >
          {addingUser ? (
            <>
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        <div className="relative flex-grow max-w-md">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <FiSearch className={`${isDark ? 'text-gray-400' : 'text-gray-400'}`} />
          </div>
          <input
            type="text"
            ...
            className={`block w-full pl-10 pr-4 py-2 rounded-md border ${isDark ? 'border-gray-600 bg-gray-700 text-white placeholder-gray-400' : 'border-gray-200 bg-white text-gray-900'} focus:ring-coquelicot-500 focus:border-coquelicot-500`}
          />
        </div>
        ...
        <div className="flex space-x-2">
          ...
          <div className="relative">
            <select
              className={`${isDark ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-700'} border py-2 px-3 pr-8 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
              value={roleFilter}
            ...
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
              <FiShield className={`${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
              <FiFilter className={`${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
            </div>
          </div>
      ...
      {error && (
        <div className={`mb-4 p-4 ${isDark ? 'bg-red-900/50 text-red-200' : 'bg-red-50 text-red-600'} rounded-md`}>
          {error}
        </div>
      )}
      ...
      <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow overflow-hidden`}>
        {loading ? (
          <div className="p-6 text-center">
            ...
            <p className={`mt-4 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Loading users...</p>
          </div>
        ) : filteredUsers.length === 0 ? (
          <div className="p-6 text-center">
            <p className={`${isDark ? 'text-gray-300' : 'text-gray-600'}`}>No users found matching your filters.</p>
          </div>
        ) : (
              ...
              <tbody className={`${isDark ? 'bg-gray-800 divide-y divide-gray-700' : 'bg-white divide-y divide-gray-200'}`}>
                {filteredUsers.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <FiMail className={`${isDark ? 'text-gray-400' : 'text-gray-400'} mr-2`} />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        {/* Role change button */}
                        <button 
                          className={`text-blue-600 p-2 rounded-full transition-colors ${isDark ? 'hover:text-blue-400 hover:bg-blue-900/30' : 'hover:text-blue-900 hover:bg-blue-50'}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            changeUserRole(user);
                          }}
                        ...
                        <button 
                          className={`p-2 rounded-full transition-colors ${
                            user.isActive 
                              ? isDark ? 'text-red-400 hover:text-red-300 hover:bg-red-900/30' : 'text-red-600 hover:text-red-800 hover:bg-red-50' 
                              : isDark ? 'text-green-400 hover:text-green-300 hover:bg-green-900/30' : 'text-green-600 hover:text-green-800 hover:bg-green-50'
                          }`}
                          onClick={(e) => {
                            e.stopPropagation();
