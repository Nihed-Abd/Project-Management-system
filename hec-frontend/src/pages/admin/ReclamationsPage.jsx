...
const ReclamationsPage = () => {
  ...
  return (
    <div className={`p-4 md:p-6 ${isDark ? 'bg-gray-900 text-white' : ''}`}>
      <div className="mb-6">
        <h1 className={`text-2xl font-semibold ${isDark ? 'text-white' : 'text-silver-100'} mb-2`}>Reclamations Management</h1>
        <p className={`${isDark ? 'text-gray-300' : 'text-silver-200'}`}>View and respond to user reclamations</p>
      </div>

      {/* Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-sm p-4 border-l-4 border-blue-500`}>
          <div className="flex items-center">
            <div className={`p-3 rounded-full ${isDark ? 'bg-blue-900/30' : 'bg-blue-100'} mr-4`}>
              <FiMessageSquare className="text-blue-500 text-xl" />
            </div>
            <div>
              <p className={`${isDark ? 'text-gray-300' : 'text-silver-200'} text-sm`}>Total Reclamations</p>
              <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-silver-100'}`}>{stats.total}</p>
            </div>
          </div>
        </div>
        
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-sm p-4 border-l-4 border-yellow-500`}>
          <div className="flex items-center">
            <div className={`p-3 rounded-full ${isDark ? 'bg-yellow-900/30' : 'bg-yellow-100'} mr-4`}>
              <FiClock className="text-yellow-500 text-xl" />
            </div>
            <div>
              <p className={`${isDark ? 'text-gray-300' : 'text-silver-200'} text-sm`}>Pending</p>
              <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-silver-100'}`}>{stats.pending}</p>
            </div>
          </div>
        </div>
        
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-sm p-4 border-l-4 border-green-500`}>
          <div className="flex items-center">
            <div className={`p-3 rounded-full ${isDark ? 'bg-green-900/30' : 'bg-green-100'} mr-4`}>
              <FiCheckCircle className="text-green-500 text-xl" />
            </div>
            <div>
              <p className={`${isDark ? 'text-gray-300' : 'text-silver-200'} text-sm`}>Answered</p>
              <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-silver-100'}`}>{stats.answered}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-sm p-4 mb-6`}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
          <div className="flex items-center space-x-2">
            <FiFilter className={`${isDark ? 'text-gray-400' : 'text-silver-200'}`} />
            <span className={`${isDark ? 'text-white' : 'text-silver-100'} font-medium`}>Filter by Status:</span>
            <div className="flex space-x-2">
              <button 
                onClick={() => handleStatusFilterChange('all')}
                className={`px-3 py-1 rounded-full text-sm ${filterStatus === 'all' 
                  ? 'bg-silver-100 text-white' 
                  : isDark ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-silver-200 hover:bg-gray-200'}`}
              >
                All
              </button>
              <button 
          ...
          <div className="relative w-full md:w-64">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <FiSearch className={`${isDark ? 'text-gray-400' : 'text-silver-200'}`} />
            </div>
            <input
              type="text"
              placeholder="Search reclamations..."
              className={`pl-10 pr-4 py-2 w-full border ${isDark ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'bg-white border-gray-200 text-gray-900'} rounded-lg focus:outline-none focus:ring-2 focus:ring-coquelicot focus:border-transparent`}
              value={searchTerm}
      ...
      <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-sm overflow-hidden`}>
        {loading ? (
          <div className={`p-4 text-center ${isDark ? 'text-gray-300' : 'text-silver-200'}`}>Loading reclamations...</div>
        ) : filteredReclamations.length === 0 ? (
          <div className="p-8 text-center">
            <FiMessageSquare className={`text-4xl ${isDark ? 'text-gray-600' : 'text-gray-300'} mx-auto mb-2`} />
            <h3 className={`text-lg font-medium ${isDark ? 'text-white' : 'text-silver-100'}`}>No reclamations found</h3>
            <p className={`${isDark ? 'text-gray-400' : 'text-silver-200'} mt-1`}>
            ...
            <table className={`min-w-full divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
              <thead className={`${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <tr>
                  <th 
                    scope="col" 
                    className={`px-6 py-3 text-left text-xs font-medium ${isDark ? 'text-gray-300' : 'text-silver-200'} uppercase tracking-wider cursor-pointer ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`}
              ...
              <tbody className={`${isDark ? 'bg-gray-800 divide-y divide-gray-700' : 'bg-white divide-y divide-gray-200'}`}>
                {filteredReclamations.map((reclamation) => (
                  <tr key={reclamation._id} className={`${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className={`text-sm font-medium ${isDark ? 'text-white' : 'text-silver-100'}`}>
                            {reclamation.userId?.name || 'Unknown User'}
                          </div>
                          <div className={`text-sm ${isDark ? 'text-gray-300' : 'text-silver-200'}`}>
                    ...
                    <td className="px-6 py-4">
                      <div className={`text-sm font-medium ${isDark ? 'text-white' : 'text-silver-100'}`}>{reclamation.object}</div>
                      <div className={`text-sm ${isDark ? 'text-gray-300' : 'text-silver-200'} truncate max-w-xs`}>
                        {reclamation.message}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
